...
{
    public class US_PrepareController : Controller
    {
        _US_Prepare prepare = new _US_Prepare();
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.PageName = "Stage: Prepare (Hoist way)";
            string MFG_No = Session["MFG"] as string;
            var result = prepare.Get_MFG_Detail(MFG_No);
       ...
       [SessionTimeout]
        ...
        {
            string MFG_No = Session["MFG"] as string;
            var result = prepare.Get_Plan_by_Stage(MFG_No, "SM_0001");
        ...
        public JsonResult GetTable_Prepare()
        {
            ...
            var result = prepare.GetList_Task_by_Stage(MFG_No, "SM_0001");
            return Json(result);
        }
        ...
        public JsonResult ToTransection(string tranNo,string TaskName)
        {
            ...
            {
                result = "Redirect",
                url = Url.Action("Transaction", "US_Prepare")
            });
        }
        #endregion
       ...
       // [SessionTimeout]
        public JsonResult GetTable_Transection(string TransectionNo)
        {
            ...
            var result = prepare.GetList_Transaction_by_Task(MFG_No, TransectionNo);
        ...
        public JsonResult ToEdit(string TT_Code)
        {
            ...
            {
                result = "Redirect",
                url = Url.Action("Edit", "US_Prepare")
            });
        }
        ...
        [SessionTimeout]
        [HttpPost]    
        public ActionResult CreatePrepare(Create_UpdateStatusVM Data)
        {
            get_roles_fn();
            ...
            var resultSave =  prepare.Save_Task_Transaction_by_Task(obj);
            ...
            if (resultSave) 
            {
                ...
                if (ListImg.Count > 0) 
                {
                    for (int i =0; i < ListImg.Count; i++) 
                    {
                        Save_Image_Transaction_by_TaskModel ImgObj = new Save_Image_Transaction_by_TaskModel()
                        {
                         ...
                         prepare.Save_Image_Transaction_by_Task(ImgObj);
                    }
                }
                 //add file to DB
                if (ListFile.Count > 0) 
                ...
                _writelog.WriteLog("Create", "US Prepare", Session["MFG"].ToString() + " => " + obj.TT_Code, false, Session["User_EmpName"] as string);
            }
            else
            {
                _writelog.WriteLog("Create", "US Prepare", Session["MFG"].ToString() + " => " + obj.TT_Code + " => " + resultSave, true, Session["User_EmpName"] as string);
            }
            //   return RedirectToAction("Transaction", "US_Prepare");
            return RedirectToAction("Index", "US_Prepare");
        }
        [SessionTimeout]
        [HttpPost]
        public ActionResult EditPrepare(Edit_UpdateStatusVM Data) 
        {
            get_roles_fn();
            ...
            for (int i = 0; i < ListImgDelete.Count; i++)
            {
                if (ListImgDelete[i] != "") //delete iumage from Database
                {
                    ...
                    prepare.Delete_Image_Transaction_by_Task(deleteobj);
                }
            }
            ...
            if (resultSave)
            {
                ...
                _writelog.WriteLog("Edit", "US Prepare", Session["MFG"].ToString() + " => " + obj.TT_Code, false, Session["User_EmpName"] as string);
            }
            else
            {
                _writelog.WriteLog("Edit", "US Prepare", Session["MFG"].ToString() + " => " + obj.TT_Code + " => " + resultSave, true, Session["User_EmpName"] as string);
            }
            // return RedirectToAction("Transaction", "US_Prepare");
            return RedirectToAction("Index", "US_Prepare");
        }
       ...
       // [SessionTimeout]
        public JsonResult GetDropDownResult()
        {
            ...
            var result = prepare.GetList_Result_by_Task(TaskCode);
       ...
       // [SessionTimeout]
        public JsonResult GetDropDownProblem()
        {
            ...
            var result = prepare.GetList_Problem_by_Task(TaskCode);
       ...
       // [SessionTimeout]
        public JsonResult Edit_GetData(Guid TransectionCode) 
        {
            ...
            var result = prepare.GetList_Transaction_by_Task(MFG_No, TaskCode)
        ...
        public JsonResult Edit_GetImg(Guid TransectionCode) 
        {
            ...
            var result = prepare.GetList_Image_Transaction_by_Task(obj)
        ...
        private void get_roles_fn()
        {
            ...
            var s_Roles = _lstGetRoles.Where(a => a.ROLE_Code == "Role_US13" || a.ROLE_Code == "Role_US14").FirstOrDefault();
            if (s_Roles != null)
