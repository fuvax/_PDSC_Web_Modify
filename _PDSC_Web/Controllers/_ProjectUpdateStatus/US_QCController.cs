...
{
    public class US_QCController : Controller
    {
        _US_QC qc = new _US_QC();
        ...
        {
            if ((Session["Stage"].ToString() == "Delivery" || Session["Stage"].ToString() == "")
                && Host.Fix_Work_Approve == 1)
            {
                return RedirectToAction("Logout", "Login");
            }
            ViewBag.PageName = "Stage: QC";
            get_roles_fn();
        ...
        public JsonResult GetPlateDate()
        {
            ...
            var result = qc.Get_Plan_by_Stage(MFG_No, "SM_0006");
        ...
        public JsonResult GetTable_Prepare()
        {
            ...
            var result = qc.GetList_Task_by_Stage(MFG_No, "SM_0006");
            return Json(result);
        }
        ...
        public JsonResult ToTransection(string tranNo, string TaskName)
        {
            ...
            {
                result = "Redirect",
                url = Url.Action("Transaction", "US_QC")
            });
        }
        #endregion
        ...
        public JsonResult GetTable_Transection(string TransectionNo)
        {
            ...
            var result = qc.GetList_Transaction_by_Task(MFG_No, TransectionNo);
        ...
        [SessionTimeout]
        [HttpPost]
        public ActionResult CreateQC(Create_UpdateStatusVM Data)
        {
            get_roles_fn();
            ...
            var resultSave = qc.Save_Task_Transaction_by_Task(obj);
            ...
            if (resultSave)
            {
                ...
                if (ListImg.Count > 0)
                {
                    for (int i = 0; i < ListImg.Count; i++)
                    {
                        ...
                        qc.Save_Image_Transaction_by_Task(ImgObj);
                    }
                }
                //add file to DB
                if (ListFile.Count > 0)
                ...
                _writelog.WriteLog("Create", "US QC", Session["MFG"].ToString() + " => " + obj.TT_Code, false, Session["User_EmpName"] as string);
            }
            else
            {
                _writelog.WriteLog("Create", "US QC", Session["MFG"].ToString() + " => " + obj.TT_Code + " => " + resultSave, true, Session["User_EmpName"] as string);
            }
            //  return RedirectToAction("Transaction", "US_QC");
            return RedirectToAction("Index", "US_QC");
        }

        [SessionTimeout]
        [HttpPost]
        public ActionResult EditQC(Edit_UpdateStatusVM Data)
        {
            get_roles_fn();
            ...
            for (int i = 0; i < ListImgDelete.Count; i++)
            {
                if (ListImgDelete[i] != "") //delete iumage from Database
                {
                    ...
                    qc.Delete_Image_Transaction_by_Task(deleteobj);
                }
            }
            ...
            if (resultSave)
            {
                ...
                _writelog.WriteLog("Edit", "US QC", Session["MFG"].ToString() + " => " + obj.TT_Code, false, Session["User_EmpName"] as string);
            }
            else
            {
                _writelog.WriteLog("Edit", "US QC", Session["MFG"].ToString() + " => " + obj.TT_Code + " => " + resultSave, true, Session["User_EmpName"] as string);
            }
        ...
        public JsonResult GetDropDownResult()
        {
            ...
            var result = qc.GetList_Result_by_Task(TaskCode);
        ...
        public JsonResult GetDropDownProblem()
        {
            ...
            var result = qc.GetList_Problem_by_Task(TaskCode);
        ...
        public JsonResult Edit_GetData(Guid TransectionCode)
        {
            ...
            var result = qc.GetList_Transaction_by_Task(MFG_No, TaskCode)
        ...
        public JsonResult Edit_GetImg(Guid TransectionCode)
        {
            ...
            var result = qc.GetList_Image_Transaction_by_Task(obj)
        ...
        private void get_roles_fn()
        {
            ...
            var s_Roles = _lstGetRoles.Where(a => a.ROLE_Code == "Role_US23" || a.ROLE_Code == "Role_US24").FirstOrDefault();
            if (s_Roles != null)
