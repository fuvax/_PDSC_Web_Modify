...
{
    public class US_DeliveryController : Controller
    {
        _US_Delivery delivery = new _US_Delivery();
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.PageName = "Stage: Delivery";
            if ((Session["Stage"].ToString() == "") && Host.Fix_Work_Approve == 1)
            {
        ...
        public JsonResult GetPlateDate()
        {
            ...
            var result = delivery.Get_Plan_by_Stage(MFG_No, "SM_0002");
        ...
        public JsonResult GetTable_Prepare()
        {
            ...
            var result = delivery.GetList_Task_by_Stage(MFG_No, "SM_0002");
            return Json(result);
        }
        ...
        public JsonResult ToTransection(string tranNo, string TaskName)
        {
            ...
            {
                result = "Redirect",
                url = Url.Action("Transaction", "US_Delivery")
            });
        }
        #endregion
        ...
        public JsonResult GetTable_Transection(string TransectionNo)
        {
            ...
            var result = delivery.GetList_Transaction_by_Task(MFG_No, TransectionNo);
        ...
        public JsonResult ToEdit(string TT_Code)
        {
            ...
            {
                result = "Redirect",
                url = Url.Action("Edit", "US_Delivery")
            });
        }
        ...
        [SessionTimeout]
        [HttpPost]
        public ActionResult CreateDelivery(Create_UpdateStatusVM Data)
        {
            get_roles_fn();
            if (ViewBag.GetRoles != "Role_US15")
                return RedirectToAction("Logout", "Login");
            ...
            var resultSave = delivery.Save_Task_Transaction_by_Task(obj);
            ...
            if (resultSave)
            {
                ...
                if (ListImg.Count > 0)
                {
                    for (int i = 0; i < ListImg.Count; i++)
                    {
                        ...
                        delivery.Save_Image_Transaction_by_Task(ImgObj);
                    }
                }
                //add file to DB
                if (ListFile.Count > 0)
                ...
                _writelog.WriteLog("Create", "US Delivery", Session["MFG"].ToString() + " => " + obj.TT_Code, false, Session["User_EmpName"] as string);
            }
            else
            {
                _writelog.WriteLog("Create", "US Delivery", Session["MFG"].ToString() + " => " + obj.TT_Code + " => " + resultSave, true, Session["User_EmpName"] as string);
            }
            // return RedirectToAction("Transaction", "US_Delivery");
            return RedirectToAction("Index", "US_Delivery");
        }
        [SessionTimeout]
        [HttpPost]
        public ActionResult EditDelivery(Edit_UpdateStatusVM Data)
        {
            get_roles_fn();
            if (ViewBag.GetRoles != "Role_US15")
                return RedirectToAction("Logout", "Login");
            ...
            for (int i = 0; i < ListImgDelete.Count; i++)
            {
                if (ListImgDelete[i] != "") //delete iumage from Database
                {
                    ...
                    delivery.Delete_Image_Transaction_by_Task(deleteobj);
                }
            }
            ...
            if (resultSave)
            {
                ...
                _writelog.WriteLog("Edit", "US Delivery", Session["MFG"].ToString() + " => " + obj.TT_Code, false, Session["User_EmpName"] as string);
            }
            else
            {
                _writelog.WriteLog("Edit", "US Delivery", Session["MFG"].ToString() + " => " + obj.TT_Code + " => " + resultSave, true, Session["User_EmpName"] as string);
            }
        ...
        {
            string TaskCode = Session["TaskNo"] as string;

            var result = delivery.GetList_Result_by_Task(TaskCode);
        ...
        public JsonResult GetDropDownProblem()
        {
            ...
            var result = delivery.GetList_Problem_by_Task(TaskCode);
        ...
        public JsonResult Edit_GetData(Guid TransectionCode)
        {
            ...
            var result = delivery.GetList_Transaction_by_Task(MFG_No, TaskCode)
        ...
        public JsonResult Edit_GetImg(Guid TransectionCode)
        {
            ...
            var result = delivery.GetList_Image_Transaction_by_Task(obj)
        ...
        private void get_roles_fn()
        {
            ...
            var s_Roles = _lstGetRoles.Where(a => a.ROLE_Code == "Role_US15" || a.ROLE_Code == "Role_US16").FirstOrDefault();
            if (s_Roles != null)
